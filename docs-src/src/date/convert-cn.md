...
- 日期解析与格式化输出使用不同的 formatter；解析采用严格模式，`2023-02-29` 等非法日期会抛出解析异常，不会被静默调整。
- `LocalDate` 转换为 `LocalDate` 时原样返回，不应用时区规则，因此不会因目标时区的跳日规则而改变日期。
...
- `LocalTime` 会原样转换为 `LocalTime`；只有显式提供固定 offset 时才能转换为 `OffsetTime`，需要日期的其他转换会被拒绝。
