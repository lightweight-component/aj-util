...
The class argument must be non-null and concrete. A null class causes `IllegalArgumentException`; interfaces are
rejected explicitly, while abstract classes fail later during constructor invocation.
...
the original failure as its cause. A null class or constructor is rejected with `IllegalArgumentException`.
...
Non-public constructors are outside this utility's supported scope. `hasArgsCon(Class<?>)` also rejects a null
class with `IllegalArgumentException`.
