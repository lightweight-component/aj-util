...
| 常用工具 | `ConvertBasicValue`、`ObjectHelper`、`MapTool`、`RandomTools`、`StrUtil`、`RegExpHelper` | 类型转换、对象、Map、随机值、字符串与带缓存的正则表达式。 |
...
| 密码学 | `DoCipher`、`AesGcm`、`AesCbc`、`AesPbe`、`AesLegacy`、`Rsa`、`SecretKeyMgr`、`RestoreKey`、`PemUtils`、`DoSignature`、`DoVerify` | AES、基于密码的加密、RSA-OAEP、密钥与签名；详见[密码学指南](/aj-util/cryptography/intro-cn/)。 |
| 反射 | `Clazz`、`Fields`、`Methods`、`NewInstance`、`Types` | 类加载、成员查找、对象构造与泛型类型检查。 |
| 日志 | `Trace`、`TextBox`、`EnableOperationLog` | MDC 链路上下文、格式化文本框与操作日志标记注解。 |

HTTP 请求、上传、下载和注解代理现属于独立的 [AJ HTTP 工程](/aj-http/cn/)。Java 包名仍为 `com.ajaxjs.util.httpremote`，但需要单独依赖；请参阅 AJ HTTP 安装指南。
