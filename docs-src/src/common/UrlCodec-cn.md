...
`simpleGET(String)` 打开 URL 流，不设置显式超时或请求头；以 UTF-8 读取，规范化换行并追加末尾分隔符，关闭输入流。URL 构造/打开失败返回 null，读取失败可能抛出 `UncheckedIOException`。需要控制请求时使用 [HTTP API](/http_request/Base-cn/)。查询串解析使用 `MapTool.toMap(String)`。
