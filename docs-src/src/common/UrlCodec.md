...
`simpleGET(String)` opens a URL stream without explicit timeouts or headers, reads UTF-8 and normalizes line endings (including a trailing separator). It closes the stream. URL/open failures return null; read failures may throw `UncheckedIOException`. Use the [HTTP API](/http_request/Base/) for controlled requests. Query parsing is `MapTool.toMap(String)`.
