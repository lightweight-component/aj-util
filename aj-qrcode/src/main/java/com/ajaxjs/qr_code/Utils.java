...
/**
 * Utility functions for QR Code generation, including image rendering and SVG output.
 */
public class Utils {
    /**
     * Returns a raster image depicting the specified QR Code with default colors (white background, black foreground).
     *
    ...
    /**
     * Writes the given buffered image to a PNG file.
     * Helper function to reduce code duplication.
     *
     * @param img      the image to write (not {@code null})
     ...
     *
     * @param x the first field element (unsigned 8-bit integer)
     * @param y the second field element (unsigned 8-bit integer)
     * @return the product of the two field elements in GF(2^8/0x11D)
     */
    public static int reedSolomonMultiply(int x, int y) {
        assert x >> 8 == 0 && y >> 8 == 0;
        int z = 0; // Russian peasant multiplication
