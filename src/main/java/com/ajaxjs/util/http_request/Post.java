...
public class Post extends Base implements HttpConstants {
    ...
    public static ResponseEntity any(String method, String url, Object params, Consumer<HttpURLConnection> fn) {
        ...
        if (params instanceof String)
            ...
            if (!map.isEmpty()) {
                String str = MapTool.join(map, v -> v == null ? null : EncodeTools.urlEncode(v.toString()));
        ...
        try {
            for (String key : data.keySet()) {
                ...
                if (v instanceof File) {
                    ...
                    _bytes = BytesHelper.concat(field.getBytes(), fileBytes);
                } else { // 普通字段
                ...
                if (bytes == null) // 第一次时候为空
                    ...
                    bytes = BytesHelper.concat(bytes, _bytes);
            }
        } catch (IOException e) {
            log.warn("Err when reading file as bytes", e);
            throw new UncheckedIOException(e);
        }

        assert bytes != null;
        return BytesHelper.concat(bytes, END_DATA);
    }
    ...
    public static Map<String, Object> postFile(String url, String fieldName, String fileName, byte[] file, Consumer<HttpURLConnection> fn) {
        ...
        byte[] bytes = BytesHelper.concat(field.getBytes(), file);   // 将字段和文件内容拼接
        bytes = BytesHelper.concat(bytes, END_DATA);// 拼接结束数据
    ...
    public static String download(String url, Consumer<HttpURLConnection> fn, String saveDir, String newFileName) {
        ...
        String fileName = Get.getFileNameFromUrl(url);
        if (newFileName != null)
