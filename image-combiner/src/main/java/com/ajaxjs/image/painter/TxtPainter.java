...
import com.ajaxjs.image.embedd.TextEmbedded;
...
public class TxtPainter implements Painter<TextEmbedded> {
    @Override
    public void draw(Graphics2D g, TextEmbedded element, int canvasWidth) {
        List<TextEmbedded> textLineElements = new ArrayList<>(); // 首先计算是否要换行（由于拆行计算比较耗资源，不设置换行则直接用原始对象绘制）
        textLineElements.add(element);

        if (element.isAutoBreakLine())
            textLineElements = element.getBreakLineElements();

        for (int i = 0; i < textLineElements.size(); i++) {
            TextEmbedded firstLineElement = textLineElements.get(0);
            TextEmbedded currentLineElement = textLineElements.get(i);

            if (!element.isAutoBreakLine() && currentLineElement.getAutoFitMaxWidth() != null) { // 自适应宽度（若超出指定宽度，缩小字体适应之，若设置了自动换行，则忽略）
            ...
            if (currentLineElement.getRotate() != null) // 绘制完后反向旋转，以免影响后续元素
                g.rotate(-Math.toRadians(currentLineElement.getRotate()),
                        currentLineElement.getX() + (double) currentLineElement.getWidth() / 2, currentLineElement.getDrawY());
        }
    }

    @Override
    public void drawRepeat(Graphics2D g, TextEmbedded element, int canvasWidth, int canvasHeight) {
