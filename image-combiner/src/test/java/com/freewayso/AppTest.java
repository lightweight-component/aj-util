package com.freewayso;

import com.ajaxjs.image.constant.enums.*;
import com.ajaxjs.image.model.enums.*;
...
public class AppTest {
    ...
    public void testRepeatOffset() throws Exception {
        ImageCombiner combiner = new ImageCombiner("https://img.thebeastshop.com/combine_image/funny_topic/resource/bg_3x4.png", OutputFormat.JPG);
        combiner.addTextElement("水印错位效果", 30, 100, 300)
                .setRepeat(true, 100, 200, -60)
                .setRotate(-15);
        combiner.combine();
        combiner.save("d://testRepeatOffset.png");
    }

    /**
     * 测试绘制连续的方框，填充数组，自动调整文字宽度
     *
     * @throws Exception
     */
    @Test
    public void testRectangles() throws Exception {
        ...
        int fontMaxWidth = 40;
        for (int i = 2; i < 12; i++) {
            combiner.addRectangleElement(offsetX, y, rectangleSize, rectangleSize).setColor(252, 196, 4).setAlpha(0.5f);
            combiner.addRectangleElement(offsetX, y, rectangleSize, rectangleSize).setBorderSize(1).setColor(Color.red);
            combiner.addTextElement(i * i * i + "", 40, offsetX + rectangleSize / 2, y + rectangleSize / 2)
                    .setBaseLine(BaseLine.Middle)
                    .setDirection(Direction.CenterLeftRight)
                    .setAutoFitWidth(fontMaxWidth);
            offsetX += rectangleSize - 1;
        }
        combiner.combine();
        combiner.save("d://testRectangles.jpg");
    }
